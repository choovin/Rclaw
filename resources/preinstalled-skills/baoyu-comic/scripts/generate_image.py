...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate images using Doubao models")
    ...
    try:
        ...
        if args.input_images:
            ...
            encoded, _ = encode_image_to_data_url(args.input_images[0])
            ...
            for attempt_model in [primary_model, fallback_model]:
                ...
                try:
                    ...
                    image_url = result_data[7:] if result_data.startswith("direct:") else poll_result(base_url, api_key, result_data)
                    last_error = None
                    break
