...
| Script | Purpose |
|--------|---------|
| `scripts/generate_image.py` | Generate images via Seedream 5.0 Lite (no external key needed) |
...
| `characters/characters.jpg` | Character reference sheet |
| `prompts/NN-{cover\|page}-[slug].md` | Generation prompts |
| `NN-{cover\|page}-[slug].jpg` | Generated images (JPEG, output by generate_image.py) |
...
- [ ] Step 7: Generate images ⚠️ CHARACTER REF REQUIRED
  - [ ] 7.1 Generate character sheet FIRST → characters/characters.jpg
  - [ ] 7.2 Generate pages WITH --input-image characters/characters.jpg
- [ ] Step 8: Merge to PDF
...
| **7.1** | **Generate character sheet FIRST** | `characters/characters.jpg` |
| **7.2** | Generate pages **with character ref** | `*.jpg` files |
...
**Image generation engine**: Uses bundled `scripts/generate_image.py` (Doubao via RunNode API). No external API key required — reads credentials from `~/.openclaw/` automatically.
...
- **Backup rule**: If `characters/characters.jpg` exists, rename to `characters/characters-backup-YYYYMMDD-HHMMSS.jpg`
- Read the Reference Sheet Prompt from `characters/characters.md` and use it as `--prompt`
```bash
# Generate character reference sheet (4:3 landscape)
python ${SKILL_DIR}/scripts/generate_image.py \
  --prompt "<Reference Sheet Prompt from characters/characters.md>" \
  --filename "characters/characters.jpg" \
  --aspect-ratio 4:3
```

**7.2 Generate each page WITH character reference**:
- Read each prompt file (`prompts/NN-*.md`) and pass its content as `--prompt`
- Pass the character sheet as `--input-image` to maintain character consistency
- **Backup rules for page generation**:
  - If image file exists: rename to `NN-{cover|page}-[slug]-backup-YYYYMMDD-HHMMSS.jpg`

```bash
# Example: generate page with character reference for consistency
python ${SKILL_DIR}/scripts/generate_image.py \
  --prompt "<full prompt text from prompts/01-page-xxx.md>" \
  --filename "01-page-xxx.jpg" \
  --aspect-ratio 3:4 \
  --input-image characters/characters.jpg
```

**Aspect ratio mapping** (baoyu-comic → generate_image.py):
| Comic option | `--aspect-ratio` value |
|---|---|
| `--aspect 3:4` (portrait, default) | `3:4` |
| `--aspect 4:3` (landscape) | `4:3` |
| `--aspect 16:9` (widescreen) | `16:9` |
| Character sheet | `4:3` |

**Output format**: JPEG (`.jpg`). Update all file references in storyboard/prompts accordingly.
