...
- **No database**: The app uses `electron-store` (JSON files) for settings and provider metadata; cloud member tokens live in a separate encrypted `electron-store` file (AES-256-GCM, machine-derived key), not the OS keychain. No database setup is needed.
