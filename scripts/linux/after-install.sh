...
# Post-installation script for RClaw on Linux
...
# Create symbolic link for RClaw app binary
if [ -x /opt/RClaw/rclaw ]; then
    ln -sf /opt/RClaw/rclaw /usr/local/bin/rclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/RClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/RClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/RClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/RClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/rclaw'
...
echo "RClaw has been installed successfully."
