...
- 技能列表来自 `useSkillsStore`：由全局策略在**进入聊天、Gateway 就绪、路由回到聊天、窗口可见/聚焦**等时机刷新（见 `docs/superpowers/specs/2026-04-07-chat-skills-auto-load-design.md`）；用户仍可在打开面板或输入 `/` 时触发 `fetchSkills()` 作为即时刷新。
- 只展示 `enabled: true` 的技能（启用态由 Gateway 与 `openclaw.json` 合并规则决定，见上述规格）。
...
### `/` 与图标共用面板（与 `2026-04-07-chat-slash-skill-picker-unify-design` 一致）

- 在合法边界处输入 `/` 时，打开**与技能图标相同**的 `SkillPickerPopover`（非独立内联下拉）；筛选字符打在面板搜索框内，composer 在对应位置仅保留 `/`；取消关闭时保留 `/`。
- **Chip 展示**：在满足下文 token **边界**规则的前提下，仅当命令名（规范化后）与某个 **已启用** 技能的 `commandName` 一致时，overlay 才渲染为 chip；否则始终为普通文本（复制、ZWSP、整块 Backspace 删除等行为与 chip 一致地仅作用于“白名单内”的 token）。
