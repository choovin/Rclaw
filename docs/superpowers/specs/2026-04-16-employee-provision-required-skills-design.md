...
3. **配置落点**
   - 使用 OpenClaw 官方 **`agents.list[].skills`** 作为每 Agent 最终技能白名单（见 [Skills Config — Agent skill allowlists](https://docs.openclaw.ai/tools/skills-config)）；省略该字段则继承 `agents.defaults.skills` / 全局默认。禁止用「关掉全局其他技能」冒充 per-agent 限制。
