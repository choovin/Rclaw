...
describe('plugin installer diagnostics', () => {
  ...
  it('retries once on Windows and logs diagnostic details when bundled copy fails', async () => {
    ...
    const sourceDir = 'C:\\Program Files\\RClaw\\resources\\openclaw-plugins\\wecom';
    const sourceManifestSuffix = 'Program Files\\RClaw\\resources\\openclaw-plugins\\wecom\\openclaw.plugin.json';
  ...
  it('logs EPERM diagnostics with source and target paths', async () => {
    ...
    expect(mockLoggerWarn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        sourceDir,
        targetDir: expect.stringMatching(/\.openclaw[\\/]extensions[\\/]wecom/i),
        platform: 'win32',
        attempts: [
